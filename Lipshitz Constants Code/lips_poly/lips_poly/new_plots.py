...
def _iterplot(variables, xlabel, ylabel, figsize, xscale, yscale, x_axis=None, legend=True):
    fig, ax = plt.subplots(figsize=figsize)
    for i, (key, value) in enumerate(variables.items()):
        if len(value) == 0:
            continue
        if xscale == 'log' and xaxis is None:
...
def _iterplots(variables, xlabel, ylabel, figsize, xscale, yscale, x_axis=None):
