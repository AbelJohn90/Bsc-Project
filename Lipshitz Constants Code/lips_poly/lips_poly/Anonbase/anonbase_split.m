function lip_prod = anonbase_split(weights, net_dims, split_amount, ...
    num_workers, alpha, mode)
    % Compute Lipschitz constant as product of constants from subnetworks
    %
    % params:
    %   weights: cell           - weights of trained neural network
    %   net_dims:Llist of ints  - neural network layer dimensions
    ...
    %   mode: str               - which Anonbase formulation to use
    %
    % returns:
    %   lip_prod: float     - Lipschitz constant of neural network

    [split_w, split_net_dims] = split_weights(weights, net_dims, split_amount);
    ...
    parfor (k = 1:num_splits, num_workers)
        ...
        if size(curr_weights, 2) > 1
            Lf_reduced_piece = anonbase_lip_multi(curr_weights, curr_net_dims, ...
